...
//	private boolean treeDecomposition = false; // sistemare.
	
	private ObjectArrayList<ResourceNode> decomp;
	...
	public TreeDecomposition(String functionClassFullName, int width, DatalogParser parser, String outputFileName) {
...
//		else
	...
	public TreeDecomposition(String datalogExpression, String atomSizes, String domainValues, String functionClassFullName, int width, boolean atomVarDistinctValues, String outputFileName) {
...
//		kvertex = new Object[KV];
		...
		if(atomVarDistinctValues)
			parser.fetchVarAtomSelectivities(domainValues);
		else
...
//		String domainValues = "testAtomVarDistinctVals.txt";
		...
		TreeDecomposition htd = new TreeDecomposition("test.txt", atomSizes, domainValues, "TDCostFunction", 6, false, "test");
//		HypertreeDecomp htd = new HypertreeDecomp("test.txt", "AGMCostFunction", 4, p, true);
...
//			printDecompTD();
		...
		if (decomp != null && checkConditionsTD()) {
...
//			root.getMinWeightResource().setWeight(root.getMinWeightResource().getBestDomainValue().toReal());
	 ...
	 * @return
	 */
	private ObjectArrayList<ResourceNode> buildDecomp() {
		// TODO Auto-generated method stub
		...
		while(stack.size() > 0) {
			...
			for(int i = 0; i < currentNode.getChildren().size(); i++) {
				...
				if(chiInclusion(currentNode, child)) {
					prune = true;
					break;
				}
			...
			if( (currentNode.getChi().size() - 1) > width)
				width = currentNode.getChi().size() - 1;
...
//				if(isIncluded(varsA, resourceNodes.get(j).getChi())) {
//					ok1 = true;
//					break;
//				}
//			
...
//		
//		for(int j = 0; j < resourceNodes.size() && ok3; j++) {
//			
//			IntArrayList chiP = resourceNodes.get(j).getChi();
//			
//			IntArrayList lambdaP = resourceNodes.get(j).getLambda();
//			
...
//					ok3 = false;
//					idHypertreeNode = resourceNodes.get(j).getId();
//					break;
//				}	
//		}
//		
...
//		boolean ok4 = true;
//		
//		for(int j = 0; j < resourceNodes.size() && ok4; j++) {
//			
//			IntArrayList chiP = resourceNodes.get(j).getChi();
//			
//			IntArrayList lambdaP = resourceNodes.get(j).getLambda();
...
//			
//			computeChiTp(resourceNodes.get(j), chiTp);
//			
...
//	private boolean checkConditionsHD(ObjectArrayList<ResourceNode> decomp) {
//		
//		/**
//		 *  1. for each atom 'A' of 'atoms(Q)',
//		 *     there exists 'p' of 'vertices(T)'
...
//	}
	...
	private boolean checkTDConditions() {
...
//		System.out.println("satisfied");
	...
	private boolean checkConditionsTD() {
...
//					else
						resourceNode.getLambda().add(mapping.getVarID(lambda[i]));
...
//						else
						resourceNode.setWeight(1); // fittizio
